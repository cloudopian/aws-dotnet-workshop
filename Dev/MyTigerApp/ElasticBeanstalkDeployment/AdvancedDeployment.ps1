...
do{
    ...
    if($appEnv -eq $null){
        ...
        New-EBEnvironment -ApplicationName $appName -EnvironmentName $appEnvName -Description "$appEnvName for $appName" -SolutionStackName "64bit Windows Server Core 2016 v1.2.0 running IIS 10.0" -OptionSetting @{Namespace="aws:autoscaling:launchconfiguration";OptionName="InstanceType";Value="t2.small"} , @{Namespace="aws:autoscaling:asg";OptionName="MaxSize";Value="2"} -VersionLabel $appVersion -Force
    ...
    if($appEnv -ne $null -and $appEnv.Status -ne "Terminating" -and $appEnv.Status -ne "Terminated"){
        ...
        Update-EBEnvironment -ApplicationName $appName -EnvironmentName $appEnvName -VersionLabel $appVersion -OptionSetting @{Namespace="aws:autoscaling:launchconfiguration";OptionName="InstanceType";Value="t2.small"} , @{Namespace="aws:autoscaling:asg";OptionName="MaxSize";Value="2"} -Description "Deployed by Azure DevOps" -Force
        while(( (Get-EBEnvironment -ApplicationName $appName -EnvironmentName $appEnvName -IncludeDeleted $false) | select -ExpandProperty Status) -ne "Ready"){
